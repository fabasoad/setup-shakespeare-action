...
main() {
  ...
  if command -v shakespeare >/dev/null 2>&1; then
    ...
    log_info "shakespeare is found at $(which shakespeare). ${msg}"
  else
    log_info "shakespeare is not found. Executing installation."
  fi
  echo "bin-installed=${bin_installed}" >> "$GITHUB_OUTPUT"

  bin_dir="shakespeare_$(date +%s)"
